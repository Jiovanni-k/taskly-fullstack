  ...
  switch (request.method) {
        ...
        result: {
          tools: [
            ...
            },
            {
              ...
              description: 'Gets a single todo item by its ID',
    ...
    case 'tools/call':
      if (toolCall.name === 'create_todo') {
        const result = await createTodoTool(toolCall.arguments);
      ...
      if (toolCall.name === 'get_todo_by_id') {
        const result = await getTodoByIdTool(toolCall.arguments);
      ...
      if (toolCall.name === 'update_todo') {
        const result = await updateTodoTool(toolCall.arguments);
      ...
      if (toolCall.name === 'delete_todo') {
        const result = await deleteTodoTool(toolCall.arguments);
