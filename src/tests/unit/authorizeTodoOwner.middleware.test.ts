...
describe("authorizeTodoOwner middleware", () => {
    ...
    it("should return 404 when the todo does not exist", async () => {
        ...
        const req = { params: { id: todoId }, user: { id: userId, role: "user" } } as unknown as Request;
        const res = mockRes();
        const next = vi.fn();
    ...
    it("should return 403 when the requester is not the owner or an admin", async () => {
        vi.mocked(prisma.todos.findUnique).mockResolvedValue({
            id: todoId, title: "x", completed: false, userId
        } as any);

        const req = { params: { id: todoId }, user: { id: otherUserId, role: "user" } } as unknown as Request;
        const res = mockRes();
        const next = vi.fn();
    ...
    it("should call next when the requester is an admin", async () => {
        ...
        const req = { params: { id: todoId }, user: { id: otherUserId, role: "admin" } } as unknown as Request;
        const res = mockRes();
        const next = vi.fn();
